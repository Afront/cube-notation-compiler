...
class Char
	...
	def isMove
		return isTurn || isSlice || isWide || isRotation
	end
	...
	def is_valid_type
		return isMove || isSymbol || isBracket
	end
	def find_type
		return "T" if self.isMove 
...
class Lexer
	...
	def lex
		...
		turns.each_char do |char|   
			c = Char.new(char)
			abort "Lexical Error: #{c.c} is not valid! (Corner cutting has not been implemented yet 😉)" if c.c == "." && !num_list.empty?
...
class Parser
	...
	def shift(symbol)
		...
		if symbol.type == "B" then  @type_stack.push(symbol.token) else @type_stack.push(symbol.type) end
	end
	...
	def rule1
		...
		while @i < @type_stack.length
			...
			if type == "T" || type == "t"
				...
				abort "Syntax Error: Integer cannot follow another Integer! " if (token.is_a? Numeric) && (@next_s.token.is_a? Numeric)
				abort "Syntax Error: An integer cannot follow an expression with both an integer and a symbol (n' and 'n)!" if @prev_is_exp	&& (@next_s.token.is_a? Numeric)
				a = @prev_is_exp
		...
		abort "Syntax Error: A prime symbol (') cannot follow an expression with an integer and a symbol (n') due to ambiguity!" if symbols.length == 3
		symbols.push 1 unless @next_s.type == "S" or not symbols.empty?
		@type_stack.push("t")
	...
	def parse
		...
		#			p "Current symbol: #{s.token} with type #{s.type}; Next symbol: #{next_s.token} with type #{next_s.type}"
			shift(s)
			@next_s =  next_s
			reduce while rule_finder(@type_stack.join) != 0 and !@stop
			@stop = false
		end
		accept
	end
...
class CodeGenerator
	...
	def simplifyToken(token_a)
		...
		if token_a.type == "t"
			...
			token_a.symbol[0].times do |time|
				token_a.token.each do |token|
					code.push simplifyToken(token)
				end
			end
